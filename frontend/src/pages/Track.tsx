...
import { AttachmentList, Banner, Card, StatusBadge } from "../components/ui";
  ...
  return (
    <PublicShell>
      <div className="mx-auto max-w-2xl space-y-5">
        ...
        {view.self_help && view.self_help.length > 0 && <SelfHelpCard materials={view.self_help} />}
...
const SELF_HELP_KIND: Record<string, string> = {
  step: "Что можно сделать",
  exercise: "Упражнение",
  read: "Полезно знать",
};

function SelfHelpCard({ materials }: { materials: SelfHelpMaterial[] }) {
  return (
    <Card className="bg-lilac-50/40">
      <div className="font-bold text-lilac-900">Пока специалист готовит ответ</div>
      <p className="mt-1 text-sm text-lilac-700/80">
        Небольшие шаги, которые могут поддержать прямо сейчас. Это не заменяет специалиста — он скоро ответит.
      </p>
      <div className="mt-3 space-y-2">
        {materials.map((m, i) => (
          <details key={i} className="group rounded-2xl border border-lilac-200 bg-white/70 p-3">
            <summary className="flex cursor-pointer items-center gap-2 font-semibold text-lilac-900 marker:content-['']">
              <span className="rounded-full bg-lilac-100 px-2 py-0.5 text-[11px] font-bold text-lilac-600">
                {SELF_HELP_KIND[m.kind] || "Совет"}
              </span>
              <span>{m.title}</span>
              <span className="ml-auto text-lilac-400 transition group-open:rotate-180">⌄</span>
            </summary>
            <p className="mt-2 whitespace-pre-wrap text-sm text-lilac-700">{m.body}</p>
            {m.url && (
              <a href={m.url} target="_blank" rel="noreferrer" className="mt-1 inline-block text-sm text-lilac-600 underline">
                Подробнее
              </a>
            )}
          </details>

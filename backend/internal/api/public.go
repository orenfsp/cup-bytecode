...
func (s *Server) handleCreateTicket(w http.ResponseWriter, r *http.Request) {
	...
	// Срочность, выставленная заявителем (по ней сортируется очередь оператора).
	...
	err = s.DB.QueryRow(`
		...
		RETURNING id`,
		hash, applicantType, categoryID, suggested, nullIfEmpty(freeText, "(без описания, выбрана категория)"), answersRaw, isCrisis, contactVal, priority,
	).Scan(&ticketID)
	if err != nil {
	...
	if s.MultipartFiles(r) != nil {
		...
		for _, fh := range files {
			...
			var saved upload.Saved
			if upload.IsAudio(ct) {
				saved, err = upload.SaveAudio(s.Cfg.UploadDir, f, ct)
			} else {
				saved, err = upload.SaveStripped(s.Cfg.UploadDir, f, ct)
			}
			f.Close()
			if err != nil {
				continue
...
func (s *Server) handleApplicantResolve(w http.ResponseWriter, r *http.Request) {
	...
	if b.Action == "not_helped" {
		...
		// Возврат уходит только оператору: снимаем эксперта и соисполнителей,
		// чтобы обращение больше не отображалось у экспертов до повторного назначения.
		s.DB.Exec(`DELETE FROM ticket_participants WHERE ticket_id=$1`, t.ID)
...
func (s *Server) applicantView(t ticket) map[string]interface{} {
	...
	// Библиотека самопомощи: пока специалист не подготовил ответ, показываем
	// короткие безопасные материалы по (предсказанной) категории обращения.
	switch t.Status {
	case "answer_ready", "done", "rejected", "closed_no_answer":
		// уже есть ответ/итог — материалы самопомощи не нужны
	default:
		slug := s.categorySlug(t.CategoryID)
		if slug == "" || slug == "unknown" {
			slug = s.categorySlug(t.SuggestedCategoryID)
		}
		view["self_help"] = selfhelp.For(slug)
	}
	return view
}

// categorySlug возвращает слаг категории по id (или "" если нет).
func (s *Server) categorySlug(id *int64) string {
	if id == nil {
		return ""
	}
	var slug string
	if err := s.DB.QueryRow(`SELECT slug FROM categories WHERE id=$1`, *id).Scan(&slug); err != nil {
		return ""
	}
	return slug
}
